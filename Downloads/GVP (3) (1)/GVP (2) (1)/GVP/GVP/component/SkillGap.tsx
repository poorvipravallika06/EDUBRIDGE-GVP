  ...
  return (
    <div className="p-6 max-w-5xl mx-auto space-y-8 animate-in fade-in duration-500">
      <div className="text-center space-y-2">
        <h2 className="text-3xl font-bold bg-gradient-to-r from-indigo-600 to-purple-600 bg-clip-text text-transparent">🎯 Skill-Gap Predictor</h2>
        <p className="text-slate-600">Compare your skills with top MNC hiring trends. (Resume must pass plagiarism check)</p>
      </div>

      <div className="grid md:grid-cols-2 gap-8">
        <div className="space-y-6 bg-gradient-to-br from-indigo-50 to-purple-50 p-8 rounded-xl shadow-lg border-2 border-indigo-300">
          <div>
            ...
            <div className="relative" ref={dropdownRef}>
              ...
              <input 
                ...
                className="pl-10 w-full p-3 bg-white text-slate-900 border-2 border-slate-300 rounded-lg focus:ring-2 focus:ring-indigo-500 focus:outline-none transition-all pr-10"
          ...
          <div>
            ...
            <div className={`border-2 border-dashed rounded-xl p-8 flex flex-col items-center justify-center text-center transition-colors relative ${resumeFile ? 'border-indigo-300 bg-indigo-100' : 'border-slate-300 hover:border-indigo-400 hover:bg-slate-50'}`}>
                <input 
                    type="file" 
                    accept=".pdf"
          ...
          <div className="bg-white p-4 rounded-lg border-2 border-orange-300 shadow-md">
            ...
            <button 
              ...
              className={`w-full py-2.5 rounded-lg font-bold flex items-center justify-center gap-2 transition-all ${
                plagiarismChecking 
                  ? 'bg-orange-300 text-white opacity-75' 
                  : plagiarismResult?.checked
        ...
        <div className="bg-gradient-to-br from-purple-50 to-pink-50 p-8 rounded-xl shadow-lg border-2 border-purple-300 min-h-[400px]">
          {!result && !loading && (
            <div className="h-full flex flex-col items-center justify-center text-slate-400">
              <div className="p-4 bg-slate-100 rounded-full mb-3">
                <Briefcase className="h-10 w-10 text-slate-300" />
              </div>
              <p className="font-medium">Results will appear here after analysis</p>
              <p className="text-xs mt-1">Complete plagiarism check first</p>
            </div>
          )}
            ...
            <div className="space-y-6 animate-in slide-in-from-bottom-4">
              <div className="flex items-center justify-between pb-4 border-b-2 border-purple-200">
                <div>
                    ...
                    <div className={`text-5xl font-bold ${result.matchScore > 75 ? 'text-green-600' : result.matchScore > 50 ? 'text-yellow-600' : 'text-red-600'}`}>
                        {result.matchScore}%
                    </div>
                </div>
                <div className="text-right max-w-[200px]">
                    <p className="text-sm text-slate-700 font-medium italic">"{result.roleFit}"</p>
                </div>
              </div>
              ...
              <div>
                ...
                <div className="grid gap-3">
                  {result.missingSkills.map((item, i) => (
                    <div key={i} className="flex items-center justify-between p-3 bg-red-50 border-2 border-red-200 rounded-lg group hover:border-red-300 transition-all shadow-sm">
                        <span className="text-sm font-bold text-red-800">{item.skill}</span>
                        <a 
                            href={item.courseLink} 
                            target="_blank" 
                            rel="noopener noreferrer"
                            className="text-xs flex items-center gap-1 bg-white text-indigo-600 px-3 py-1.5 rounded-md border-2 border-indigo-200 hover:bg-indigo-50 hover:border-indigo-300 transition-colors shadow-sm font-bold"
                        >
              ...
              <div>
                ...
                <ul className="space-y-2 text-sm text-slate-600">
                  {result.recommendations.map((rec, i) => (
                    <li key={i} className="flex items-start gap-2 bg-slate-50 p-3 rounded-lg border border-slate-200 hover:border-slate-300 transition-all">
