...
import { getMockMentorResponse } from "./getMockMentorResponse";
...
// Mock mentor responses imported from getMockMentorResponse.ts
...
export const sendMessageToMentor = async (history: {role: string, parts: {text: string}[]}[], message: string, language: string = 'English') => {
  try {
    ...
    Language: RESPOND ONLY IN ${language}. Use proper ${language} language and grammar.
    ...
    Format: Use ** for bold, • for bullet points. Make responses easy to read.
    If off-topic: Politely decline in ${language}.`;
