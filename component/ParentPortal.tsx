...
const ParentPortal: React.FC = () => {
    ...
    // Validation function for phone numbers (10 digits only)
    ...
    const handleStudentMobileChange = (e: React.ChangeEvent<HTMLInputElement>) => {
        const value = e.target.value.slice(0, 10); // Max 10 digits
        setStudentMobile(value);
        if (value && !validatePhoneNumber(value)) {
            setStudentMobileError('invalid phonenumber');
        } else {
            setStudentMobileError('');
        }
    };
    ...
    const handleParentMobileChange = (e: React.ChangeEvent<HTMLInputElement>) => {
        ...
        if (value && !validatePhoneNumber(value)) {
            setParentMobileError('invalid phonenumber');
        } else {
            setParentMobileError('');
        }
    };
    ...
    const handleParentEmailChange = (e: React.ChangeEvent<HTMLInputElement>) => {
        ...
        if (value && !validateEmail(value)) {
            setParentEmailError('mail id invalid');
        } else {
            setParentEmailError('');
        }
    };
    ...
    const handleVerify = (e: React.FormEvent) => {
        ...
        if (!studentMobile || !validatePhoneNumber(studentMobile)) {
            setStudentMobileError('invalid phonenumber');
            alert('Student mobile must be 10 digits');
            return;
        }
        if (!parentMobile || !validatePhoneNumber(parentMobile)) {
            setParentMobileError('invalid phonenumber');
            alert('Parent mobile must be 10 digits');
            return;
        }
    ...
    const handleSendWithDoc = async () => {
        ...
        try {
            ...
            setSending(false);
            setSent(true);
            setParentEmailError(''); // Clear error on success
                        ...
                        <div>
                             ...
                             <div className="relative">
                                 ...
                                 <input 
                                     ...
                                     maxLength={10}
                                     className={`w-full pl-9 p-2 bg-slate-50 border-2 rounded-lg text-sm focus:ring-2 focus:outline-none transition-all ${
                                         studentMobileError ? 'border-red-500 focus:ring-red-500' : 'border-slate-300 focus:ring-indigo-500 focus:border-indigo-500'
                                     }`}
                                     placeholder="9876543210"
                                 />
                             </div>
                             {studentMobileError && <p className="text-xs text-red-500 mt-1">{studentMobileError}</p>}
                        </div>
                        ...
                        <div>
                             ...
                             <div className="relative">
                                 ...
                                 <input 
                                     ...
                                     maxLength={10}
                                     className={`w-full pl-9 p-2 bg-slate-50 border-2 rounded-lg text-sm focus:ring-2 focus:outline-none transition-all ${
                                         parentMobileError ? 'border-red-500 focus:ring-red-500' : 'border-slate-300 focus:ring-indigo-500 focus:border-indigo-500'
                                     }`}
                                     placeholder="9876543210"
                                 />
                             </div>
                             {parentMobileError && <p className="text-xs text-red-500 mt-1">{parentMobileError}</p>}
                        </div>
                            ...
                            <div className="mb-4">
                                ...
                                <div className="relative">
                                    ...
                                    <input 
                                        ...
                                        className={`w-full pl-10 p-2.5 border-2 rounded-lg text-sm focus:ring-2 focus:outline-none transition-all ${
                                            parentEmailError ? 'border-red-500 focus:ring-red-500 focus:border-red-500' : 'border-slate-300 focus:ring-indigo-500 focus:border-indigo-500'
                                        }`}
                                        disabled={sending || sent}
                                    />
                                </div>
                                {parentEmailError && <p className="text-xs text-red-500 mt-1">{parentEmailError}</p>}
                                {sent && (
